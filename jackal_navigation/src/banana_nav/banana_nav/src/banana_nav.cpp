...
int buffer = 99;//Determines what cost values are considered objects
	 ...
	 ROS_INFO("Height = %d",height);
	   if(LTtrigger == false){
		for(width = halfWidth; width>=0;width--) {//search for the closet tree/obstacle to the left of the base_link
				...
				if ((LTcost > buffer)){//found a lethal cost and haven't found one before this.									//lethal points are trees and it WILL NOT see last layers trees
					LTtrigger = true;
	   ...
	   if(RTtrigger == false){
		for(width = halfWidth; width <= maxWidth;width++){//search for the closet tree/obstacle to the right of the base_link
				...
				if ((RTcost > buffer)){//found a lethal cost and haven't found one before this.									//lethal points are trees and it WILL NOT see last layers trees
					RTtrigger = true;
